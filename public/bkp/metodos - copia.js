...
queryModel.getEsquema = function(unEsquema,callback){
	if (connection) 
	{
		var sql= 'select id,esquema from rio2015.esquemas where nombre = \''+unEsquema+'\'';
...
queryModel.getComponente = function(unCompo,callback){
	if (connection){
		var sql= 'select id,componente from rio2015.componentes where nombre = \''+unCompo+'\'';
		connection.query(sql, function(error, row){
			if(error){
				row= '';
				error = error + sql;
...
queryModel.getMetrica = function(unaMetrica,callback){
	if (connection){
		var sql= 'select id,metrica,valor,operacion from rio2015.metricas where nombre = \''+unaMetrica+'\'';
		connection.query(sql, function(error, row){
			if(error){
				row= '';
				error = error + sql;
...
queryModel.listarEsquemas = function(username,callback){
	if (connection){
...
queryModel.listarComponentes = function(username,callback){
	if (connection){
...
queryModel.listarMetricas = function(username,callback){
	if (connection){
...
queryModel.buscarUsuario = function(usuario,callback){
	if (connection){
		var sql = 'select username from rio2015.users where username = \''+usuario+'\'';
		connection.query(sql,function(error,result){
			if (result==null){
				callback('');				
			}
...
queryModel.obtenerUsuario = function(username,callback){
	if (connection){
		var sql = 'select username from rio2015.users where username = \''+username+'\'';
		connection.query(sql,function(error, result){
			if (!result || error){
				callback(error+''+sql, '');
			}
...
queryModel.listarUsuarios = function(callback){
	if (connection){
		var sql = 'select username from rio2015.users';
		connection.query(sql,function(error,result){
		//Asumo que no hay errores en la consulta
			callback(null,result);			
		});
...
queryModel.editarComponenteAsignado = function (componente,callback){
	if (connection){
		//TO DO.
	}
}

queryModel.listarCubos = function (callback){
	if (connection){
		var sql = 'select * from rio2015.aprovisionadores';
		connection.query(sql,function(error,result){
			if (error){
				callback(error,null);
			}
