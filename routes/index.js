...
var RedisClient = require ('../public/consultasRedis');
...
module.exports = function(app){
	...
	app.get("/esquema/query/:esq",function(req,res){
		...
		RedisClient.obtenerEsquema(esquema,function(error, data){
	...
	app.get("/componentes/buscar/:componente",function(req,res){
		...
		RedisClient.obtenerComponente(unComponente,function(error, data){
	...
	app.get("/metricas/buscar/:metrica",function(req,res){
		...
		RedisClient.obtenerMetrica(unaMetrica,function(error, data){
	...
	app.get("/esquemas/listar/:user",function(req,res){
		...
		RedisClient.listarExtractores(username,function(error,data){
			res.json(data);
		});		
	});	
	...
	app.get("/componentes/listar/:user",function(req,res){
		var username= req.params.user;
		RedisClient.listarComponentes(username,function(error,data){
			if (typeof data !== 'undefined' && data.length > 0){
				res.json(data);
			}
			//en otro caso enviamos un json vacio
	...
	app.get("/metricas/listar/:user",function(req,res){
		var username= req.params.user;
		RedisClient.listarMetricas(username,function(error,data){
			if (typeof data !== 'undefined' && data.length > 0){
				res.json(data);
			}
			//en otro caso enviamos un json vacio
...
//si el componente se ha eliminado correctamente mostramos su info
	...
	app.get("/login/:user",function (req,res){
		...
		RedisClient.buscarUsuario(username,function(data){
			if(typeof data !== 'undefined' && data.length > 0){
	...
	app.get("/usuarios/obtener/:user",function(req,res){
		RedisClient.obtenerUsuario(req.params.user,function(error,data){
	...
	app.get("/usuarios/listar",function(req,res){
		...
		RedisClient.listarUsuarios(function(error,data){			
			if(typeof data !== 'undefined' && data.length > 0){
				res.json(data);
			}
			else{
	...
	app.get("/cubos/listar",function (req,res){
		...
		RedisClient.listarAprovisionadores(function(error,data){
			if(typeof data !== 'undefined' && data.length > 0){
				res.json(data);
			}
			else{
	...
	app.get("/cubos/buscar/:nombre",function (req,res){
		...
		RedisClient.buscarAprovisionador(req.params.nombre,function(error, data){
	...
	app.get("/cubos/buscarCampos/:user",function (req,res){
		
		Query.buscarCuboUsuario(req.params.user,function(error, data){
			//si el Aprovisionador existe lo mostramos en formato json
			if (typeof data !== 'undefined' && data.length > 0){
				res.json(data);
			}
			//en otro caso enviamos un json vacio
				...
				console.log('ERROR_GET_APROVISIONADOR_USR: '+error);
				res.json({});
			}		
		});
	});	
	...
	app.get("/asignaciones/cubos/",function (req,res){
		
		Query.consultarAsignacionCubos(function(error,data){
			//si hay asignaciones las retornamos en formato json
			if (typeof data !== 'undefined' && data.length > 0){
				res.json(data);
			}
			//en otro caso enviamos un json vacio
			else{
				console.log('ERROR_ASIG_APROV: '+error);
				res.json({});
			}		
		});
	});
	
	app.get("/asignaciones/extractores/",function (req,res){
		
		Query.consultarAsignacionExt(function(error,data){
			//si hay asignaciones las retornamos en formato json
			if (typeof data !== 'undefined' && data.length > 0){
				res.json(data);
			}
			//en otro caso enviamos un json vacio
				...
				console.log('ERROR_ASIG_EXT: '+error);
				res.json({});
			}		
		});
	});	
	
	app.get("/asignaciones/componentes/",function (req,res){
		
		Query.consultarAsignacionCompo(function(error,data){
			//si hay asignaciones las retornamos en formato json
			if (typeof data !== 'undefined' && data.length > 0){
				res.json(data);
			}
			//en otro caso enviamos un json vacio
				...
				console.log('ERROR_ASIG_COMPONENTES: '+error);
				res.json({});
			}		
		});
	});	
	
	app.get("/asignaciones/metricas/",function (req,res){
		
		Query.consultarAsignacionMetri(function(error,data){
			//si hay asignaciones las retornamos en formato json
			if (typeof data !== 'undefined' && data.length > 0){
				res.json(data);
			}
			//en otro caso enviamos un json vacio
				...
				console.log('ERROR_ASIG_METRICAS: '+error);
				res.json({});
			}		
		});
	});

	app.put("/aprovisionadores/desasignar/:aprov/:user",function(req,res){
		
		Query.desasignarAprovisionador(req.params.aprov,req.params.user,function(error,data){
			//si la asignacion se modifico mostramos su info
			if(data && data.insertId){
				console.log('Asignacion-Aprov id: '+data.insertId+' modificado con exito');
			}
			else{
				console.log('ERROR_DESASIG_APROV: '+error);
				res.json({"msg":"ERROR_DESASIGNAR_APROV: "+error});
			}					
		});
	});
	
	app.put("/extractores/desasignar/:ext/:user",function(req,res){
		
		Query.desasignarExtractor(req.params.ext,req.params.user,function(error,data){
			//si la asignacion se modifico mostramos su info
			if(data && data.insertId){
				console.log('Asignacion-Ext: '+data.insertId+' modificado con exito');
			}
			else{
				console.log('ERROR_DESASIG_EXT: '+error);
				res.json({"msg":"ERROR_DESASIGNAR_EXT: "+error});
			}					
		});
	});	
	
	app.put("/componentes/desasignar/:compo/:user",function(req,res){
		
		Query.desasignarComponente(req.params.compo,req.params.user,function(error,data){
			//si la asignacion se modifico mostramos su info
			if(data && data.insertId){
				console.log('Asignacion-Compo: '+data.insertId+' modificado con exito');
			}
			else{
				console.log('ERROR_DESASIG_COMPO: '+error);
				res.json({"msg":"ERROR_DESASIGNAR_COMPO: "+error});
			}					
		});
	});

	app.put("/metricas/desasignar/:metrica/:user",function(req,res){
		
		Query.desasignarMetrica(req.params.metrica,req.params.user,function(error,data){
			//si la asignacion se modifico mostramos su info
			if(data && data.insertId){
				console.log('Asignacion-Metrica: '+data.insertId+' modificada con exito');
			}
			else{
				console.log('ERROR_DESASIG_MET: '+error);
				res.json({"msg":"ERROR_DESASIGNAR_MET: "+error});
			}					
		});
	});	
